...
namespace Zor.BehaviorTree.Core.Leaves.Conditions
{
		...
		where T : class, IComparable<T>
	{
		...
		{
			if (blackboard.TryGetClassValue(m_referenceValuePropertyName, out T referenceValue) &
				blackboard.TryGetClassValue(m_comparedValuePropertyName, out T comparedValue))
			{
				Status* results = stackalloc Status[] {Status.Failure, Status.Success};
				bool isGreater = referenceValue != null && referenceValue.CompareTo(comparedValue) > 0;

...
{
	public sealed class CalculateNavMeshPath : Action,
		...
		ISetupable<string, string, string>
	{
		...
		{
			SetupInternal(sourcePropertyName, targetPropertyName, pathPropertyName);
		}
			...
			string pathPropertyName)
		{
			SetupInternal(new BlackboardPropertyName(sourcePropertyName),
				new BlackboardPropertyName(targetPropertyName), new BlackboardPropertyName(pathPropertyName));
		}
		...
		protected override Status Execute()
		{
				...
				blackboard.TryGetStructValue(m_targetPropertyName, out Vector3 target))
			{
				...
				bool found = NavMesh.CalculatePath(source, target, NavMesh.AllAreas, path);
